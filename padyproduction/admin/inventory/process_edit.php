...
$errors = [];
if ($id <= 0)          $errors[] = "ID item tidak valid.";
...
// Jangan izinkan quantity baru lebih kecil dari yang sedang terpakai
if ($quantity < (int)$item['quantity_in_use']) {
    $_SESSION['inv_errors'] = [
        "Jumlah baru ($quantity) tidak boleh lebih kecil dari jumlah yang sedang dipakai ({$item['quantity_in_use']})."
    ];
    header("Location: edit.php?id=$id");
    exit();
}
